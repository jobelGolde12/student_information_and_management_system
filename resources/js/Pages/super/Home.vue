...
import LayoutForSuper from '@/Layouts/LayoutForSuper.vue';
   ...
   <LayoutForSuper>
    <div class="mt-2  container">
        ...
        <div class="container-fluid d-flex justify-content-end mb-3">
            ...
            <Link :href="route('add.instructor')" class="btn btn-secondary me-2"> Add Instructor</Link>
            <Link :href="route('create.admin')" class="btn btn-info"> Add Admin</Link>
        </div>
        ...
        <div class="table table-striped table-responsive pb-5">
            ...
            <tbody class="t-body">
                <tr v-for="data in tableData" :key="data.index">
                    ...
                    <td>{{ data.role }}</td>
                    <td>
                        <button class="btn btn-primary me-2 " :class="{'disabled' : data.role === 'superadmin'}" data-bs-toggle="modal" data-bs-target="#edit" @click="getData(data)">Edit</button>
                        <button class="btn btn-warning" :class="{'disabled' : data.role === 'superadmin'}" data-bs-toggle="modal" data-bs-target="#exampleModal" @click="getIdToDelete(data.id)">Delete</button>
                    </td>
                </tr>
            </tbody>
        </div>
